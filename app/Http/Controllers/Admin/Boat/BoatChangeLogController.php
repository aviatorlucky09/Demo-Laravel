...
class BoatChangeLogController extends Controller
{
 public function __construct()
 {
    ...
    $arr['plural_name'] = "Boat Change Logs";
    $arr['table_name'] = "change_logs";
  
    $this->config_arr =$arr;

}
public function index(Request $request,$boat_id)
{
    ...
    return view($view_path,compact('carr','deleted','active_tab','boat'));
...
public function processing(Request $request){
       ...
       if($request->deleted == 1){
        ...
        $query_obj->where('class_name','Boat')->where('class_id',$boatId);
