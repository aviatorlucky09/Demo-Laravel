  ...
  <div class="col-md-10">
    ...
    <div class="row">
      <div class="col-md-2">
        <div id="{{ $day_key }}_{{ $part_key }}_check_div" class="form-check">
          ...
          <input class="form-check-input" type="checkbox" name="arr[{{ $day_key }}][{{ $part_key }}]" id="{{ $day_key }}_{{ $part_key }}_check" @if(getBoatPriceValue($prices,$day_key,$part_key) == 1) checked @endif value="1">
      ...
      <div class="col-md-2">
       <div id ="{{ $day_key }}_{{ $part_key }}_price_div" class="input-group mb-3">
        ...
        @endphp
        <input type="text" class="form-control time pr-2" name="arr[{{ $day_key }}][{{ $field_name }}]" id="{{ $day_key }}_{{ $field_name }}" value="{{ getBoatPriceValue($prices,$day_key,$field_name) }}">
      </div>
    </div>
    <div class="col-md-2">
      <div id="{{ $day_key }}_{{ $part_key }}_start_hr_div" class="input-group bootstrap-timepicker timepicker time">
        ...
        @endphp
        <input id="{{ $day_key }}_{{ $field_name }}" type="text" class="form-control input-small time_picker pr-2 timepicker-24-hr" name="arr[{{ $day_key }}][{{ $field_name }}]" 
        value="{{ getBoatPriceValue($prices,$day_key,$field_name) }}">
    ...
    <div class="col-md-2">
     <div id="{{ $day_key }}_{{ $part_key }}_end_hr_div" class="input-group bootstrap-timepicker timepicker time">
       ...
       @endphp
       <input id="{{ $day_key }}_{{ $field_name }}" type="text" class="form-control input-small time_picker pr-2"  name="arr[{{ $day_key }}][{{ $field_name }}]" value="{{ getBoatPriceValue($prices,$day_key,$field_name) }}">
       <span class="input-group-addon"><i class="far fa-clock text-dark"></i></span>
     </div>
   </div>
   <div id="{{ $day_key }}_{{ $part_key }}_turnaround_div" class="col-md-2">
     ...
     @endphp
     <input id="{{ $day_key }}_{{ $field_name }}" type="number" step="0.01" name="arr[{{ $day_key }}][{{ $field_name }}]" class="form-control input-smal" value="{{ getBoatPriceValue($prices,$day_key,$field_name) }}">
   </div>
 </div>
 @endforeach
</div>
