...
<div class="row">
  <ul id="image_sortable" style="width: 100%">
    ...
    <li data_id="{{ $image->id }}" class="col-md-4 col-xl-3">
      ...
      action="{{ route('admin.marina_image_delete',['marina_id'=>$marina->id,'image_id'=>$image->id]) }}" 
    ...
    <div class="switcher pull-left pt-2">
      <input type="checkbox" name="status" id="status_{{ $image->id }}" onclick="return updateImageStatus(this)"
       data-url = "{{ route('admin.marina_image_status_store',['marina_id'=>$marina->id,'image_id'=>$image->id])}}" @if($image->status == 1) checked @endif  value="1" >
      <label for="status_{{ $image->id }}"></label>
    </div>
    <br /><br />
    @php
    $path = 'storage/uploads/marina/'.$marina->id."/".$image->image;
    @endphp
...
<script>
  $( function() {
    var pos_arr = [];
    var url = "{{ route('admin.marina_image_position_update',['marina_id'=>$marina->id]) }}";
